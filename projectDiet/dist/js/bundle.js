...
//Tabs
    ...
    tabsParent.addEventListener('click', (event) => {
        ...
        if (target && target.classList.contains(tabsSelector.slice(1))) {
            //метод слайс удалит у нас . в обозначение класса и сразу передаст нам нужное значение, поскольку мы начали с второго символа в аргументе
            tabs.forEach((item, i) => {
                if (target === item) {
                    hideTabContent()
                    showTabContent(i)
...
window.addEventListener('DOMContentLoaded', () => {
    const modalTimerId = setTimeout(()=>(0,_modules_modal__WEBPACK_IMPORTED_MODULE_1__.openModal)('.modal',modalTimerId), 1000000)
